...
{
    // You must initialize this array with the exact 781 Zobrist keys used by Polyglot.
    // For brevity, we omit the actual initialization data here.
    // In a real implementation, you would fill this array with the known Polyglot keys.
    private static readonly ulong[] zobristKeys = new ulong[781]
    {
    ...
    public ulong ComputeZobristKeyFromFen(string fen)
    {
        // Parse FEN
        ...
        int enPasRank = -1;
        int enPasFile = -1;
        bool pawnNearby = false;

        // Start from empty hash
        ulong hash = 0UL;
        ...
        if (!string.IsNullOrEmpty(enPassant) && enPassant != "-" )
        {
            ...
            {
                enPasFile = enPassant[0] - 'a';
                enPasRank = enPassant[1] - '1';
                if (enPasFile < 0 || enPasFile > 7 || enPasRank < 0 || enPasRank > 7)
                    error = true;
            }
            else
            {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            foreach (char c in ranks[i])
            {
                ...
                {
                    int pieceIndex = PieceToZobristIndex(c);
                    if (pieceIndex == -1)
                    {
                        error = true;
                        break;
                    ...
                    // pieceIndex * 64 + squareIndex
                    int squareIndex = rank * 8 + file;
                    int zobIndex = pieceIndex * 64 + squareIndex;
                    hash ^= zobristKeys[zobIndex];

                    // Check pawn proximity for en passant
                    if (char.ToLower(c) == 'p' && enPasFile != -1 && enPasRank != -1)
                    {
                        // If black pawn just moved two squares forward from rank 1 to 3, enPasRank=2
                        // White pawns that can capture are on rank 4 (since black moved from rank 1 to 3)
                        // Similarly for white pawns and black capturing logic.
                        if (char.IsUpper(c) && enPasRank == 5 && rank == 4 && 
                            ((enPasFile > 0 && file == enPasFile - 1) || (enPasFile < 7 && file == enPasFile + 1)))
                        {
                            pawnNearby = true;
                        }
                        if (char.IsLower(c) && enPasRank == 2 && rank == 3 && 
                            ((enPasFile > 0 && file == enPasFile - 1) || (enPasFile < 7 && file == enPasFile + 1)))
                        {
                            pawnNearby = true;
                        }
            ...
            if (file != 8) error = true;
        }

        // En passant file if applicable and pawn is nearby
        if (enPasFile != -1 && pawnNearby)
        {
            // En passant keys start at index 772 and go through 779 (8 files)
            hash ^= zobristKeys[772 + enPasFile];
        }

        // Side to move
        // side keys: white side key at index 780
        // If sideToMove == 'w', XOR with zobristKeys[780], if black do nothing
        if (sideToMove == "w")
        {
        ...
        // Castling
        // Indices: K=768, Q=769, k=770, q=771
        if (!string.IsNullOrEmpty(castling) && castling != "-")
        {
            if (castling.Contains('K')) hash ^= zobristKeys[768];
            if (castling.Contains('Q')) hash ^= zobristKeys[769];
            if (castling.Contains('k')) hash ^= zobristKeys[770];
            if (castling.Contains('q')) hash ^= zobristKeys[771];
        }

        if (error)
            throw new ArgumentException("Invalid FEN or error during hashing");

        return hash;
    }

    // Convert piece character to polyglot piece index:
    // Polyglot indexing (0-based for each piece type):
    // p=0, P=1, n=2, N=3, b=4, B=5, r=6, R=7, q=8, Q=9, k=10, K=11.
    private static int PieceToZobristIndex(char c)
    {
        switch (c)
        {
            case 'p': return 0;
    ...
    public string FindBookMove(ulong zobristKey, string bookPath)
    {
        // Each entry in a polyglot book is 16 bytes:
        // [0..7]: key (big-endian 64-bit)
        // [8..9]: move (big-endian 16-bit)
        // [10]: weight
        // [11]: learn
        // [12..15]: unused
        // Entries are sorted by key. We'll perform a binary search for the given key.

        using (var fs = new FileStream(bookPath, FileMode.Open, FileAccess.Read))
        using (var br = new BinaryReader(fs))
        {
            ...
            while (low <= high)
            {
                ...
                ulong key = ReadBigEndianUInt64(br);
                
                if (key == zobristKey)
                {
                    // Found at least one matching entry, but there may be multiple.
                    // We need to find all entries with this key.
                    // Move backward to find the first entry with this key.
                    long start = mid;
                    while (start > 0)
                    {
                        fs.Seek((start - 1) * 16, SeekOrigin.Begin);
                        ulong prevKey = ReadBigEndianUInt64(br);
                        if (prevKey == zobristKey)
                            start--;
                        else
                            break;
                    }

                    // Move forward from start to collect all moves
                    var moves = new System.Collections.Generic.List<(ushort move, byte weight)>();
                    long pos = start;
                    while (pos * 16 < fs.Length)
                    {
                        fs.Seek(pos * 16, SeekOrigin.Begin);
                        ulong k2 = ReadBigEndianUInt64(br);
                        if (k2 != zobristKey)
                            break;

                        ushort move = ReadBigEndianUInt16(br);
                        byte weight = br.ReadByte();
                        byte learn = br.ReadByte(); // Not used
                        br.ReadInt32(); // skip unused

                        moves.Add((move, weight));
                        pos++;
                    }

                    var random = new Random();
                    var bestMove = moves[random.Next(moves.Count)].move;

                    return PolyglotMoveToUci(bestMove);
                }
                else if (key < zobristKey)
                {
                    low = mid + 1;
                }
                else
    ...
    private static string PolyglotMoveToUci(ushort move)
    {
        // Polyglot move encoding:
        // Bits:
        //  from: bits 0-5 (6 bits)
        //  to:   bits 6-11 (6 bits)
        //  promotion: bits 12-15 (4 bits)
        // decode squares into file/rank
        int fromSquare = move & 0x3F;        // bits 0-5
        int toSquare = (move >> 6) & 0x3F;   // bits 6-11
        int promote = (move >> 12) & 0xF;    // bits 12-15

        string uci = CastleFix(SquareToString(toSquare) + SquareToString(fromSquare));
        if (promote != 0)
        {
            // 1=Knight,2=Bishop,3=Rook,4=Queen in polyglot
            char pc = 'q';
            if (promote == 1) pc = 'n';
            else if (promote == 2) pc = 'b';
            else if (promote == 3) pc = 'r';
            else if (promote == 4) pc = 'q';
    ...
    private static string CastleFix(string move)
    {
        switch (move)
        {
            case "e1h1": return "e1g1"; // White Kingside Castling
            case "e1a1": return "e1c1"; // White Queenside Castling
            case "e8h8": return "e8g8"; // Black Kingside Castling
            case "e8a8": return "e8c8"; // Black Queenside Castling
            default: return move;         // Non-castling moves remain unchanged
        }
    }
